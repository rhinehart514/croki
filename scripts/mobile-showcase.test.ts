...
it("maps capture scenes to the real application routes", () => {
  assert.equal(showcaseSceneUrl("threads", "environment-1"), "croki-dev://");
  assert.equal(
    showcaseSceneUrl("environments", "environment-1"),
    "croki-dev://settings/environments",
  );
  assert.equal(
    showcaseSceneUrl("thread", "environment-1"),
    "croki-dev://threads/environment-1/remote-command-center",
  );
  assert.equal(
    showcaseSceneUrl("terminal", "environment-1"),
    "croki-dev://threads/environment-1/remote-command-center/terminal?terminalId=term-1",
  );
  assert.equal(
    showcaseSceneUrl("review", "environment-1"),
    "croki-dev://threads/environment-1/remote-command-center/review",
  );
});
