...
export const SHOWCASE_PROJECT_ID = "croki";
...
const SHOWCASE_TERMINAL_PROMPT =
  "\u001b[1;32m→\u001b[0m \u001b[1;36mcroki\u001b[0m \u001b[1;34mgit:(\u001b[1;31mfeat/remote-command-center\u001b[1;34m)\u001b[0m \u001b[1;33m✗\u001b[0m ";
...
export const SHOWCASE_TERMINAL_BUFFER = [
  ...
  "  \u001b[32m→\u001b[0m  \u001b[2mProject:\u001b[0m  \u001b[1mcroki\u001b[0m \u001b[2m— ~/Code/croki\u001b[0m",
  "",
...
const PROJECT_FAVICONS = {
  croki: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128">
...
export const SHOWCASE_PROJECTS = [
  {
    ...
    title: "Croki",
    directory: "croki",
    repositoryUrl: "https://github.com/rhinehart514/croki.git",
    favicon: PROJECT_FAVICONS.croki,
  },
  {
    id: "react",
    title: "React",
...
async function seedCrokiWorkspace(workspaceRoot: string): Promise<void> {
  ...
  await NodeFSP.writeFile(
    ...
    `${JSON.stringify({ name: "croki", private: true, scripts: { test: "vp test" } }, null, 2)}\n`,
  );
  await NodeFSP.writeFile(NodePath.join(workspaceRoot, "favicon.svg"), PROJECT_FAVICONS.croki);
  await NodeFSP.writeFile(
...
async function seedCompanionWorkspace(input: {
  ...
  await NodeFSP.writeFile(
    ...
    `# ${input.title}\n\nSeeded by the Croki mobile screenshot harness.\n`,
  );
  await initializeRepository({
...
export async function seedShowcaseEnvironment(input: {
  ...
  if (primaryProject.id === SHOWCASE_PROJECT_ID) {
    await seedCrokiWorkspace(workspaceRoot);
  }
  await Promise.all(
    projects
