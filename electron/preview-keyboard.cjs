// Chromium CDP key packets for the agent-drivable preview, using the same required fields and
// down-event choice as Playwright's pinned Chromium keyboard implementation.
...
// Chromium does not infer macOS editing commands from synthetic Meta chords. Keep the common
// browser editing/navigation shortcuts explicit so dispatched key events behave like their
// physical-key equivalents.
...
const SHORTCUT_MODIFIER_ORDER = ["Shift", "Control", "Alt", "Meta"];
...
function modifierMask(modifiers) {
  return (modifiers ?? []).reduce((value, modifier) => {
    switch (modifier) {
      case "Alt": return value | 1;
      case "Control": return value | 2;
      case "Meta": return value | 4;
      case "Shift": return value | 8;
      default: return value;
    }
  }, 0);
}

function resolveKeyDefinition(input) {
  ...
  if (printable) {
    const shifted = input.modifiers?.includes("Shift") ?? false;
    const key = printable.shiftedKey && (shifted || input.key === printable.shiftedKey)
...
/** Build the CDP keyDown/keyUp packet pair for one pressed key with optional modifiers. */
function makePreviewKeySequence(input, options = {}) {
...
module.exports = { makePreviewKeySequence };
