// Firm tool-safety guard — the single source of truth for which tool NAMES may ever be handed to a
...
//
// Safety by construction: the teammate toolset has no approve / send / publish / deploy / charge tool,
// and no idea kill/keep/verdict tool. The only path past a founder wall is the founder resolving it
...
// wall. The teammate may `ideate` (generate + pause), but deciding which idea lives or becomes work is
// resolved off the agent surface (resolveFirmIdeas), never by a tool the model can call. This guards
...
export function assertSafeTool(name) {
  if (FORBIDDEN_TOOL.test(name)) {
    throw new Error(`Refusing to expose teammate tool "${name}" to a subprocess: it matches an outbound/approval verb.`);
  }
  if (FORBIDDEN_IDEA_VERDICT.test(name)) {
    throw new Error(`Refusing to expose teammate tool "${name}" to a subprocess: killing or keeping an idea is a founder act, not an agent tool.`);
  }
  return name;
}
