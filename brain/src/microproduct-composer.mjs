...
- THE WALL (non-negotiable). You PRODUCE the files. You never deploy, publish, push, commit, or go live, and you never write a script whose purpose is to auto-deploy. The files are built into an isolated worktree and held behind the founder gate; they ship ONLY after an explicit founder approval — "vibe up to the gate, never past it." This holds identically for shape (B): an in-repo change is written into an isolated branch off the real repo and stops before commit, never touching the founder's working tree.
