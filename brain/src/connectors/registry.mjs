...
// Legacy pipeline template (for backward compat)
export function defaultTemplate() {
  const DRAFT_PROMPT = `Write a short, warm, personalized cold outreach message.
    ...
    channels: [
      {
        ...
        label: "Cold email",
        stages: [
          { id: "find",  type: "find",  connector: "exa",     label: "Find with Exa",     config: { limit: 8 }, agentPrompt: "" },
          { id: "enrich",type: "enrich",connector: "clay",    label: "Enrich with Clay",  config: {}, agentPrompt: "" },
          { id: "score", type: "score", connector: "default", label: "Score ICP fit",     config: { minFitScore: 0.3, keywords: ["founder","startup","saas","analytics","b2b"] }, agentPrompt: "" },
          { id: "draft", type: "draft", connector: "claude",  label: "Draft outreach",    config: { senderName: "Jacob", model: "claude-haiku-4-5-20251001" }, agentPrompt: DRAFT_PROMPT },
          { id: "gate",  type: "gate",  connector: "default", label: "Review gate",       config: {}, agentPrompt: "" },
        ],
      },
    ],
    context: { senderName: "Jacob", productContext: "GTM IDE — a local workbench that proves your GTM funnel" },
  };
}
