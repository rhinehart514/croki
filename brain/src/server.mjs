...
// The prior guard (A4) refused an approval stamped `x-gtm-actor: agent`, but that trusts the caller to
// incriminate itself with the header. A capable agent holding raw loopback HTTP can POST an approval with
// NO header and be treated as the founder — the brain server has no auth otherwise. This closes that
// residual hole with a capability the real page holds and a header-less raw caller does not: a per-process
// random token, minted server-side at startup and handed ONLY to a browser that actually loaded a page
// (delivered as an HttpOnly, SameSite=Strict cookie on GET responses). An approval must carry it or it is
// refused — INDEPENDENT of the agent header, so a blind raw-curl POST with no cookie cannot release.
//
...
// caller that never loaded a page in this process lifetime never received the cookie.
...
function requestHasSessionToken(req) {
