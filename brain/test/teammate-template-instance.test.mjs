...
// cross-venture graduation path (prove a lesson in two distinct instances, then bless it up into the
...
function templateWithGraduatedLesson(ref, text, opts) {
  ...
  for (const ventureId of ["seed-a", "seed-b"]) {
    store.record(ventureId, ref, { text, taskId: "t1" }, { templateRef: ref }, opts);
    store.record(ventureId, ref, { text, taskId: "t2" }, { templateRef: ref }, opts);
    store.record(ventureId, ref, { text, taskId: "t2" }, { templateRef: ref }, opts);
    store.promote(ventureId, ref, key, {}, opts);
  }
...
describe("template on the bench, redefined per venture", () => {
  it("births an instance whose soul inherits the template's graduated lessons", () => {
    withRoot((opts) => {
      ...
      const instance = store.ensure("brand-new-venture", "outreach-writer", { templateRef: "outreach-writer" }, opts);
      assert.equal(instance.ventureId, "brand-new-venture");
  ...
  it("births thin when the ref has no template in the library", () => {
    withRoot((opts) => {
      const instance = store.ensure("solo-venture", "brand-new-ref", { templateRef: "brand-new-ref" }, opts);
  ...
  it("does not double-count an inherited lesson as ready to graduate", () => {
    withRoot((opts) => {
      ...
      store.ensure("fresh-venture", "outreach-writer", { templateRef: "outreach-writer" }, opts);
      // The lesson lives in `soul`, never in `learnings`, so it never resurfaces as a pending graduation.
      assert.deepEqual(store.listReady("fresh-venture", "outreach-writer", {}, opts), []);
    });
  });
  ...
  it("promoting a locally-relearned pattern that matches an inherited entry is idempotent", () => {
    withRoot((opts) => {
      ...
      // The founder happens to correct the same thing three times in this venture too.
      store.record("fresh-venture", "outreach-writer", { text: "no em-dashes ever", taskId: "r1" }, {}, opts);
      store.record("fresh-venture", "outreach-writer", { text: "no em-dashes ever", taskId: "r2" }, {}, opts);
      store.record("fresh-venture", "outreach-writer", { text: "no em-dashes ever", taskId: "r2" }, {}, opts);
      assert.equal(store.listReady("fresh-venture", "outreach-writer", {}, opts).length, 1);

      const promoted = store.promote("fresh-venture", "outreach-writer", key, {}, opts);
  ...
  it("exposes the cross-venture readiness check for an instance without needing the templateRef by hand", () => {
    withRoot((opts) => {
      const key = patternKeyFor("lead with their trigger");
      // Prove the lesson in one instance and bless it there. One venture alone is not enough to graduate
      ...
      // Prove and bless it in a second venture — now the template is ready, surfaced from either instance.
      ...
      assert.equal((store.get(LIBRARY_VENTURE, "prospect-researcher", opts)?.soul ?? []).length, 0);
    });
  });
});
