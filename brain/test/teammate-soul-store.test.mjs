...
// Every test runs against an isolated on-disk root, so the real ~/.gtm-ide is never touched and the
...
describe("teammate-soul-store: birth + round-trip", () => {
  ...
  it("gives projectmates distinct names", () => {
    withRoot((opts) => {
  ...
  it("scopes souls per project — the same teammate in two projects is two souls", () => {
    withRoot((opts) => {
      store.ensure("p1", "outreach-writer", {}, opts);
      store.ensure("p2", "outreach-writer", {}, opts);
      assert.equal(store.listForProject("p1", opts).length, 1);
      assert.equal(store.listForProject("p2", opts).length, 1);
    });
  });
});
...
describe("teammate-soul-store: graduation through the store", () => {
  ...
  it("surfaces a ready lesson and promotes it on the founder's blessing", () => {
    withRoot((opts) => {
      ...
      const promoted = store.promote("rodentradar", "outreach-writer", key, { run: 8 }, opts);
      assert.equal(promoted.soul.length, 1);
      assert.equal(promoted.soul[0].graduatedAtRun, 8);
  ...
  it("folds real outcomes into the track record", () => {
    withRoot((opts) => {
      store.recordOutcome("rodentradar", "outreach-writer", { runs: 1, sent: 1, replies: 1 }, {}, opts);
      const soul = store.get("rodentradar", "outreach-writer", opts);
      assert.deepEqual(soul.record, { runs: 1, sent: 1, replies: 1, wins: 0 });
    });
  });
});
...
describe("teammate-soul-store: cross-project (template) graduation", () => {
  function promoteInProject(projectId, opts) {
    store.record(projectId, "outreach-writer", { text: "no em-dashes ever", taskId: "a" }, { templateRef: "outreach-writer" }, opts);
    store.record(projectId, "outreach-writer", { text: "no em-dashes ever", taskId: "b" }, { templateRef: "outreach-writer" }, opts);
    store.record(projectId, "outreach-writer", { text: "no em-dashes ever", taskId: "b" }, { templateRef: "outreach-writer" }, opts);
    store.promote(projectId, "outreach-writer", patternKeyFor("no em-dashes ever"), {}, opts);
  }

  it("graduates a lesson up to the template once it's proven in 2 projects", () => {
    withRoot((opts) => {
      promoteInProject("rodentradar", opts);
      // One project alone does not graduate the template.
      assert.deepEqual(store.templateReady("outreach-writer", {}, opts), []);
      promoteInProject("strelva", opts);
