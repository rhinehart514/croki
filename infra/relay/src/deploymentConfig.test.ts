...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("croki-relay-traces", "prod")).toBe("croki-relay-traces-prod");
    expect(relayResourceNameForStage("croki-relay-traces", "dev_julius")).toBe(
      "croki-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "crokirelay.com", hash)).toBe(
      "prod-abcdef0123456789.crokirelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "crokirelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });
