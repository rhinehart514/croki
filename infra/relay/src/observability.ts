...
export const RelayObservability = Effect.gen(function* () {
  ...
  const traces = yield* Axiom.Dataset("RelayTracesDataset", {
    name: relayResourceNameForStage("croki-relay-traces", stage),
    kind: "otel:traces:v1",
    description: "Croki relay Worker HTTP request spans.",
    retentionDays: 30,
    useRetentionPeriod: true,
  });

  const workerIngestToken = yield* Axiom.ApiToken("RelayWorkerAxiomIngestToken", {
    name: relayResourceNameForStage("croki-relay-otel-ingest", stage),
  ...
  const mobileIngestToken = yield* Axiom.ApiToken("RelayMobileAxiomIngestToken", {
    name: relayResourceNameForStage("croki-mobile-otel-ingest", stage),
    description: "Owned by Alchemy. Scoped OTLP ingest token for Croki mobile spans.",
    datasetCapabilities: Output.map(traces.name, (dataset) => ({
      [dataset]: { ingest: ["create" as const] },
    })),
  });
  ...
  const clientIngestToken = yield* Axiom.ApiToken("RelayClientAxiomIngestToken", {
    name: relayResourceNameForStage("croki-relay-client-otel-ingest", stage),
  ...
  yield* Axiom.View("RelayRecentSpansView", {
    name: relayResourceNameForStage("croki-relay-recent-spans", stage),
...
export const makeRelayTraceLayer = (input: {
  ...
  Layer.effect(
    ...
    OtlpTracer.make({
      ...
      resource: {
        serviceName: "croki-relay-worker",
        attributes: {
