...
describe("reconcileRootEnvRelayUrl", () => {
  ...
  it("preserves unrelated root env entries while replacing a previous relay URL", () => {
    expect(
      reconcileRootEnvRelayUrl(
        "CROKI_CLERK_PUBLISHABLE_KEY=pk_test_example\nCROKI_RELAY_URL=https://old.example.test\n",
...
describe("reconcileRootEnvPublicConfig", () => {
  const config = {
    ...
    mobileTracingUrl: "https://api.axiom.co/v1/traces",
    mobileTracingDataset: "croki-mobile-traces-dev",
    mobileTracingToken: "xaat-public-ingest",
    clientTracingUrl: "https://api.axiom.co/v1/traces",
    clientTracingDataset: "croki-relay-client-traces-dev",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "CROKI_MOBILE_OTLP_TRACES_DATASET=croki-mobile-traces-dev",
        "CROKI_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "CROKI_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "CROKI_RELAY_CLIENT_OTLP_TRACES_DATASET=croki-relay-client-traces-dev",
        "CROKI_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
    ...
    expect(
      reconcileRootEnvPublicConfig(
        [
          "CROKI_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "CROKI_RELAY_URL=https://old.example.test",
          "CROKI_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "CROKI_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "CROKI_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "CROKI_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "CROKI_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "CROKI_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "CROKI_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "CROKI_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "CROKI_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
