...
- Two or more same-turn snapshots carrying explicit concept metadata become one ranked concept set in the canonical Thread. Preserve every captured alternative up to the bounded limit and the model's initial order; never infer concepts from ordinary snapshots or auto-collapse them into synthesized directions. Reordering is direct drag manipulation with an equivalent keyboard path; Keep/Question/Reject and two-up comparison remain reversible local exploration. Continue and Remix must place a visible, editable selection in the native composer and never send it automatically.
