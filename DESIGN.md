...
- **Finish mode** is an explicit Codex-only durable outcome attached to the current Thread. The founder supplies one objective and may optionally bound token use; Croki shows Codex's native active, paused, blocked, limited, and complete states and never invents percent-complete progress. Finish mode may continue native turns, but it does not create a Croki task board, hidden planning loop, or provider-neutral imitation.
