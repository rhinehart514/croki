...
`.croki/application.croki` is the repository-owned application brief. It
records three bounded kinds of founder-approved truth:

- who and what the application is for;
...
agent may create or edit it only when the founder explicitly asks for or
confirms the product delta.

## Compatibility and failure

Legacy `.croki/application.json` and historical Concept, Release, and Venture
files remain readable compatibility input. They do not become active
navigation or product objects.

All reads are bounded, validated, escaped, and nonblocking. Missing, invalid,
unreadable, unsupported, or oversized data never prevents a native provider
...
Croki Senses derive read-only Perception Frames from native project activity.
Canvas may project those frames for the founder, but it is not a `.croki`
editor, application database, release board, context authoring surface, memory
store, or provider harness.

The legacy `.croki/context.json` format and its node-authoring model remain
