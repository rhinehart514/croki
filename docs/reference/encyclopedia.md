...
This is a living glossary for Croki. Compatibility package and protocol names
may retain T3 identifiers even when the visible product is Croki.
...
A Git worktree used as an isolated workspace for a thread. If a thread has a `worktreePath` in [the contracts][1], it runs there instead of in the main working tree. Git operations are coordinated by [GitManager.ts][3].
