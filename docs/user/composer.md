...
Messages can contain up to 120,000 characters. If a draft is longer, Croki keeps it in the
...
also reports that skill as a native slash command, Croki hides the duplicate native entry and keeps
the `/skill:Skill Name` label.

On desktop, press `Cmd+Enter` on macOS or `Ctrl+Enter` on Windows and Linux from a new thread to
start it in the background. Croki opens another new thread and shows an **Open** action for the
