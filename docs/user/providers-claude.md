...
This guide is for people who want to use more than one Claude setup in Croki. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Croki](./install.md).
...
In Croki Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Croki uses Claude Code's normal config directory.

When you set this field, Croki points Claude Code at that directory with the
...
On web and desktop, when you return to an older Claude thread with a large context, Croki
...
Croki looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where Croki looks.

Then add another Claude provider in Croki:
...
Croki only offers Claude providers that use the same config directory for an existing Thread. A
...
multiple files under its config directory, so Croki keeps separate config directories isolated
...
Add or edit a Claude provider in Croki Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Croki stores the value as a server secret and does not
...
Croki does not need a special Claude Code Router provider. Treat the router as a Claude
