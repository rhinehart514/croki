...
| Outcome/result/learning stores | Keep and connect | They close the market-to-product loop. |
| MCP front door | Keep and re-center | It is the bridge into AI coding sessions. |
...
- pipeline count as a primary product-health signal;
- agent nodes that only represent mechanical execution steps;
- any dashboard that reports activity without changing a GTM or product decision.

## Delete or do not reintroduce

- the old DDD program/policy/profile/instance/foundry model;
- fixed GTM stage skeletons;
- closed GTM channel enums;
- mandatory pre-run programs or contracts;
- agent creation policies as a host-side domain layer;
...
```text
Audit the current repository against docs/production-direction/00-index.md and this disposition.

Produce a file-by-file map of what is kept, changed, demoted, deleted, or left untouched. Do not make a
rename-only migration. Trace each proposed change to a real user-visible product shift. Preserve unrelated
