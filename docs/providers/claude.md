...
This guide is for people who want to use more than one Claude setup in Croki.
...
An empty `Claude HOME path` means Croki uses your normal home directory.
...
Croki only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so Croki keeps separate Claude homes isolated instead of
...
2. Add or configure a Claude provider in Croki.
