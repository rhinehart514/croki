...
Croki connects directly to your Git hosting provider so you can create pull requests, review code, and manage repositories without leaving your editor. Work stays in flow—no more jumping between browser tabs and terminal windows.
...
1. Install the GitHub CLI on the machine running Croki:
   ```bash
   brew install gh
   ```
2. Sign in:
...
2. Add these environment variables to the environment running Croki:
   ```bash
   export CROKI_BITBUCKET_EMAIL="you@example.com"
   export CROKI_BITBUCKET_API_TOKEN="your-token"
   ```
3. Restart Croki and verify the connection in **Source Control settings**
