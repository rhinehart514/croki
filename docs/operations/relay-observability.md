...
- `croki-relay-traces-prod`, an OpenTelemetry trace dataset for Worker requests
- `croki-relay-otel-ingest-prod`, a dataset-scoped ingest token bound to the Worker
- `croki-relay-recent-spans-prod`, a view of recent request and endpoint spans

Alchemy stages append their sanitized stage name to isolate resources, for example
`croki-relay-traces-dev-julius` for a personal stage.
