...
test("dense generated maps contain 120 earned records without desktop overflow", async () => {
  ...
  try {
    ...
    const switched = await client.evaluate(`(() => {
      const tab = [...document.querySelectorAll('.venture-map-tabs [role="tab"]')].find((entry) => entry.textContent.trim() === 'Go-to-market');
      tab?.click();
      return Boolean(tab);
    })()`);
    assert.equal(switched, true);
    await waitForDom(client, `document.querySelector('.venture-maps h1')?.textContent?.trim() === 'Go-to-market'`, "dense GTM graph did not render");
