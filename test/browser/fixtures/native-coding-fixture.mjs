...
export async function seedNativeCoding({ root }) {
  ...
  const venture = createVenture({ name: "Drover native coding", repository: ROOT }, options);
  const runtime = {
    id: "codex", label: "Codex", supportsAbort: true, costReporting: "none",
    async drive(ctx) {
      ctx.onRuntimeSession("browser-provider-session");
      fs.writeFileSync(path.join(ctx.cwd, "native-coding-browser-proof.txt"), "implemented in an isolated Drover workspace\n");
  ...
  const interrupted = openCodingWorkspace({
    ventureId: venture.id,
    runId: "drive-browser-interrupted",
