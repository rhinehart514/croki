  ...
  try {
    ...
    const normal = await client.evaluate(`(() => {
      ...
      return {
        ...
        pages: document.querySelectorAll('.product-gtm-node[data-role="page"]').length,
        unrelated: document.querySelectorAll('.product-gtm-node:is([data-kind="branch"], [data-kind="action"], [data-role="workflow-step"])').length,
    ...
    assert.ok(normal.pages > 0 && normal.nodes < 126, JSON.stringify(normal));
    assert.equal(normal.unrelated, 0, `the resting Product walk leaked unrelated venture machinery: ${JSON.stringify(normal)}`);
    ...
    await waitForDom(client, `document.querySelectorAll('.product-gtm-node[data-role="workflow-step"]').length === 12 && Boolean(document.querySelector('.product-gtm-surface .react-flow'))`, "the focused full play did not survive the 200% zoom remount");
    const zoomed = await client.evaluate(`Promise.all([
      ...
      fullPlay: document.querySelectorAll('.product-gtm-node[data-role="workflow-step"]').length,
