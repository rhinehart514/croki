...
- `.croki/application.croki` is optional repository-owned application lineage; `.croki/application.json` is read-only migration compatibility. Released reality and building intent are founder-approved facts, not an agent-maintained memory. Never silently establish, promote, or rewrite them.
