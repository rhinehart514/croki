---
name: test-croki-app
description: Launch, retain, and test the Croki web app in isolated development environments, including first-try browser authentication with one-time pairing URLs, pairing-token recovery, worktree-safe state directories, cross-turn dev server lifecycle, and direct SQLite inspection or fixture seeding. Use when an agent needs to run T3 locally, iteratively test UI behavior with a human, recover from an expired or consumed pairing token, isolate dev state, or prepare test data in state.sqlite.
---

# Test T3 App

Use this skill for the web client. For iOS Simulator, Android Emulator, or physical-device testing against an isolated T3 backend, use the sibling [`test-croki-mobile`](../test-croki-mobile/SKILL.md) skill.
...
2. Choose a base directory that belongs only to the current worktree or test:
   ...
   - Use `mktemp -d /tmp/croki-test.XXXXXX` for disposable state and retain the printed absolute path.
...
The worktree-local default deliberately outranks an ambient `CROKI_HOME`; do not pass the shared home through to a worktree dev server.
...
Use the `Pair URL` from this command once. Derive `<server-port>` and `<web-url>` from the current dev-runner output, including any automatically selected port offset. Setting `CROKI_PORT` keeps the administrative CLI from probing for an unrelated free port.
