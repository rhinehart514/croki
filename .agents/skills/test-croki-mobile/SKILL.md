...
- App: `Croki Dev`
- Bundle/package identifier: `com.croki.croki.dev`
- URL scheme: `croki-dev`
...
Use direct SQLite mutation only for disposable projection fixtures. Follow `test-croki-app` and stop the backend before writing.
...
1. Inspect any process on the intended Metro port and its `/status` response. Reuse it only when it is healthy, belongs to this worktree, and matches `APP_VARIANT=development`, `--dev-client`, and scheme `croki-dev`.
...
- Workspace: `<repo>/apps/mobile/ios/CrokiDev.xcworkspace`
- Scheme: `CrokiDev`
- Configuration: `Debug`
- Simulator ID: the selected UDID
- Bundle ID: `com.croki.croki.dev`

Check the installed client with:

```bash
xcrun simctl get_app_container <simulator-udid> com.croki.croki.dev app
...
```bash
adb -s <emulator-serial> shell pm path com.croki.croki.dev
...
```bash
CROKI_PORT=<server-port> node apps/server/src/bin.ts auth pairing create \
  --base-dir <base-dir> \
...
In PowerShell, set `$env:CROKI_PORT = "<server-port>"` first and run the `node ... auth pairing create` command without the leading assignment.
...
```bash
xcrun simctl openurl <simulator-udid> 'croki-dev://connections/new'
adb -s <emulator-serial> shell am start -W \
  -a android.intent.action.VIEW \
  -d 'croki-dev://connections/new' \
  com.croki.croki.dev
```

Run only the command for the selected platform.

In Croki Dev, open Add Environment and enter the complete `<mobile-origin>` and newly printed `Token`. Verify the expected seeded projects appear before exercising the affected flow.
...
Use `snapshot_ui` and current element references from XcodeBuildMCP for taps and typing. Stream the same UDID through `ios-simulator-browser` so the user can watch in Croki when the host supports it. Use the stream as a visual feed rather than a reason to switch to fragile browser coordinates.
...
3. Remove the disposable environment from Croki Dev.
