...
const runtimePaths = (baseDir: string, version: string) => {
  ...
  return {
    versionDir,
    entryPath: NodePath.join(versionDir, "node_modules", "croki-server", "dist", "bin.mjs"),
...
export class Launcher {
  ...
  async #startChild(version: string, role: ChildRole, update?: ServiceUpdateRecord): Promise<void> {
    ...
    if (!(await runtimeExists(this.#baseDir, version))) {
      throw new Error(`Selected croki-server@${version} runtime is missing or incomplete.`);
    }
    if (this.#stopping) return;
...
async function main(): Promise<void> {
  ...
  if (baseDir === undefined || baseDir === "") {
    throw new Error("T3CODE_HOME is required by the Croki service launcher.");
  }
