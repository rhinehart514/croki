...
import { VcsDriverKind, type VcsDriverKind as VcsDriverKindType } from "@croki/contracts";
...
>()("croki-server/vcs/VcsProjectConfig") {}
...
export const make = Effect.gen(function* () {
  ...
  const findConfigPath = Effect.fn("VcsProjectConfig.findConfigPath")(function* (cwd: string) {
    let current = cwd;
    while (true) {
      const candidate = path.join(current, ".croki", "vcs.json");
