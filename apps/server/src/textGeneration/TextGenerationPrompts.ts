...
import type { ChatAttachment } from "@croki/contracts";
...
function preserveMessageEnd(message: string): string {
  ...
  const contents = alreadyTruncated
    ...
    : message;
  if (!alreadyTruncated && contents.length <= 8_000) return contents;
...
export function buildThreadTitlePrompt(input: ThreadTitlePromptInput) {
  ...
  const prompt = buildPromptFromMessage({
    instruction: isRegeneration
      ? [
          "Regenerate the title for an existing Croki thread so the user can recognize it weeks later.",
          `The previous title was ${JSON.stringify(input.previousTitle)}.`,
        ].join("\n")
      : "Generate a title that will help the user recognize this Croki thread weeks later.",
    ...
    message: input.message,
    ...(isRegeneration
      ? { messageLabel: "Thread contents", preserveMessageEnd: true }
      : {}),
    attachments: input.attachments,
