  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        'args="$*"',
        'stdin_content="$(cat)"',
        'if [ -n "$CROKI_FAKE_CLAUDE_ARGS_MUST_CONTAIN" ]; then',
        '  printf "%s" "$args" | grep -F -- "$CROKI_FAKE_CLAUDE_ARGS_MUST_CONTAIN" >/dev/null || {',
        '    printf "%s\\n" "args missing expected content" >&2',
        "    exit 2",
        "  }",
        "fi",
        'if [ -n "$CROKI_FAKE_CLAUDE_ARGS_MUST_NOT_CONTAIN" ]; then',
        '  if printf "%s" "$args" | grep -F -- "$CROKI_FAKE_CLAUDE_ARGS_MUST_NOT_CONTAIN" >/dev/null; then',
        '    printf "%s\\n" "args contained forbidden content" >&2',
        "    exit 3",
        "  fi",
        "fi",
        'if [ -n "$CROKI_FAKE_CLAUDE_STDIN_MUST_CONTAIN" ]; then',
        '  printf "%s" "$stdin_content" | grep -F -- "$CROKI_FAKE_CLAUDE_STDIN_MUST_CONTAIN" >/dev/null || {',
        '    printf "%s\\n" "stdin missing expected content" >&2',
        "    exit 4",
        "  }",
        "fi",
        'if [ -n "$CROKI_FAKE_CLAUDE_CONFIG_DIR_MUST_BE" ] && [ "$CLAUDE_CONFIG_DIR" != "$CROKI_FAKE_CLAUDE_CONFIG_DIR_MUST_BE" ]; then',
        '  printf "%s\\n" "CLAUDE_CONFIG_DIR was $CLAUDE_CONFIG_DIR" >&2',
        "  exit 5",
        "fi",
        'if [ -n "$CROKI_FAKE_CLAUDE_STDERR" ]; then',
        '  printf "%s\\n" "$CROKI_FAKE_CLAUDE_STDERR" >&2',
        "fi",
        'printf "%s" "$CROKI_FAKE_CLAUDE_OUTPUT"',
        'exit "${CROKI_FAKE_CLAUDE_EXIT_CODE:-0}"',
        "",
      ].join("\n"),
    );
  ...
  return Effect.gen(function* () {
    ...
    yield* Effect.acquireRelease(
      Effect.sync(() => {
        ...
        process.env.CROKI_FAKE_CLAUDE_OUTPUT = input.output;
