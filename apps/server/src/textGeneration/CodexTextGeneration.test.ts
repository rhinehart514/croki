...
import { CodexSettings, ProviderInstanceId, TextGenerationError } from "@croki/contracts";
...
const CodexTextGenerationTestLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
  prefix: "croki-codex-text-generation-test-",
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "  cat > \"$output_path\" <<'__CROKI_FAKE_CODEX_OUTPUT__'",
        input.output,
        "__CROKI_FAKE_CODEX_OUTPUT__",
        "fi",
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "croki-codex-text-" });
...
it.layer(CodexTextGenerationTestLayer)("CodexTextGeneration", (it) => {
  ...
  it.effect("uses CROKI_CODEX_LAUNCH_ARGS for codex exec over settings", () =>
    withFakeCodexEnv(
      {
        output: JSON.stringify({
          subject: "Add important change",
        ...
        environment: { CROKI_CODEX_LAUNCH_ARGS: " --strict-config --listen off " },
        requireArg: "--strict-config",
