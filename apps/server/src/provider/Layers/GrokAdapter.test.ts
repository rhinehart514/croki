...
const grokAdapterTestLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "croki-grok-adapter-test-",
...
it.layer(grokAdapterTestLayer)("GrokAdapterLive", (it) => {
  ...
  it.effect("completes a Grok turn from xAI prompt completion when the prompt RPC hangs", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          CROKI_ACP_EMIT_XAI_PROMPT_COMPLETE_THEN_HANG: "1",
          CROKI_ACP_EMIT_FOREIGN_SESSION_UPDATES: "1",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
  ...
  it.effect("does not report a synthetic stop reason when xAI omits one", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          CROKI_ACP_EMIT_XAI_PROMPT_COMPLETE_THEN_HANG: "1",
          CROKI_ACP_OMIT_XAI_PROMPT_COMPLETE_STOP_REASON: "1",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
  ...
  it.effect("lets Stop unblock a fully silent Grok prompt and accept a follow-up turn", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          CROKI_ACP_HANG_FIRST_PROMPT_FOREVER: "1",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
  ...
  it.effect("does not let a cancelled prompt settlement consume the follow-up prompt slot", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          CROKI_ACP_HANG_FIRST_PROMPT_FOREVER: "1",
          CROKI_ACP_REQUEST_LOG_PATH: requestLogPath,
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
  ...
  it.effect("drops late ACP notifications after a turn is cancelled", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          CROKI_ACP_HANG_PROMPT_FOREVER: "1",
          CROKI_ACP_EMIT_LATE_UPDATE_AFTER_CANCEL: "1",
        }),
      );
    ...
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          CROKI_ACP_REQUEST_LOG_PATH: requestLogPath,
          CROKI_ACP_EMIT_TOOL_CALLS: "1",
          CROKI_ACP_ALLOW_ONCE_OPTION_ID: "agent-defined-approval-id",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
      const eventsFiber = yield* Stream.runForEach(adapter.streamEvents, (event) =>
  ...
  it.effect("handles xAI ask_user_question extension requests", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({ CROKI_ACP_EMIT_XAI_ASK_USER_QUESTION: "1" }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
      const requested =
