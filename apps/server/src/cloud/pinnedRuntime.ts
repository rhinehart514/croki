...
/**
 * A pinned runtime is an exact `croki-server@<version>` npm-installed into
 ...
 * switching over, never `npx croki-server`, whose cache is ephemeral and whose
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  return yield* Effect.gen(function* () {
    ...
    yield* runner
      .run({
        command: "npm",
        args: ["install", "--prefix", stagingDir, "--no-fund", "--no-audit", `croki-server@${input.version}`],
