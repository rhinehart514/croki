import {
  ...
  type OrchestrationQueuedTurnStart,
...
// The projection table uses one pending placeholder for the start currently
// being opened, followed by durable FIFO placeholders for later ordinary
// sends.  The placeholder is an internal binding detail, so read models expose
// only the actual next-turn queue while a provider session is starting.
const visibleQueuedTurnStarts = (
  pending: ReadonlyArray<OrchestrationQueuedTurnStart>,
  session: OrchestrationSession | null,
): ReadonlyArray<OrchestrationQueuedTurnStart> =>
  session?.status === "starting" && session.activeTurnId === null ? pending.slice(1) : pending;
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  const getBackgroundLivenessShellField = (
    threadId: ThreadId,
  ): Partial<Pick<OrchestrationThreadShell, "backgroundLiveness">> => {
    const backgroundLiveness = threadBackgroundLiveness.getThreadBackgroundLiveness(threadId);
    return backgroundLiveness === null ? {} : { backgroundLiveness };
  };
  ...
  const listThreadRowsByProject = SqlSchema.findAll({
  ...
  const listPendingTurnStartRows = SqlSchema.findAll({
    ...
    execute: () =>
      sql`
        ...
        ORDER BY thread_id ASC, requested_at ASC, pending_message_id ASC
      `,
  });

  const listThreadSessionRowsByProject = SqlSchema.findAll({
    Request: ProjectIdLookupInput,
    Result: ProjectionThreadSessionDbRowSchema,
    execute: ({ projectId }) =>
      sql`
  ...
  const listCheckpointRowsByProject = SqlSchema.findAll({
    Request: ProjectIdLookupInput,
    Result: ProjectionCheckpointDbRowSchema,
    execute: ({ projectId }) =>
      sql`
  ...
  const listLatestTurnRowsByProject = SqlSchema.findAll({
    Request: ProjectIdLookupInput,
    Result: ProjectionLatestTurnDbRowSchema,
    execute: ({ projectId }) =>
      sql`
  ...
  const listPendingTurnStartRowsByThread = SqlSchema.findAll({
    Request: ThreadIdLookupInput,
    Result: ProjectionPendingTurnStartDbRowSchema,
    execute: ({ threadId }) =>
      sql`
          ...
          ([
            ...
            Effect.gen(function* () {
              ...
              const pendingTurnStartsByThread = new Map<
                string,
                Array<OrchestrationQueuedTurnStart>
              >();
              ...
              for (const row of pendingTurnStartRows) {
                updatedAt = maxIso(updatedAt, row.requestedAt);
                const pending = pendingTurnStartsByThread.get(row.threadId) ?? [];
                pending.push({
                ...
                pendingTurnStartsByThread.set(row.threadId, pending);
              }
  ...
  const getProjectPerception: NonNullable<ProjectionSnapshotQueryShape["getProjectPerception"]> = (
    input,
  ) =>
    sql
      .withTransaction(
        Effect.gen(function* () {
          const [projectRow, threadRows, sessionRows, checkpointRows, latestTurnRows, revisionRow] =
            yield* Effect.all([
              ...
              listThreadRowsByProject({ projectId: input.projectId }),
              listThreadSessionRowsByProject({ projectId: input.projectId }),
              listCheckpointRowsByProject({ projectId: input.projectId }),
              listLatestTurnRowsByProject({ projectId: input.projectId }),
  ...
  const getThreadDetailByIdBounded = (threadId: ThreadId, bounds: ThreadDetailBounds | undefined) =>
    Effect.gen(function* () {
      const [
        ...
        listPendingTurnStartRowsByThread({ threadId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getThreadDetailById:listPendingTurnStarts:query",
              "ProjectionSnapshotQuery.getThreadDetailById:listPendingTurnStarts:decodeRows",
            ),
          ),
        ),
      ]);
