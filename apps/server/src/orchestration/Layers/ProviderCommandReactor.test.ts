...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const service: ProviderServiceShape = {
      ...
      uploadFeedback: () => Effect.die(new Error("Unsupported provider call in test")) as never,
      get streamEvents() {
  ...
  it("does not inject legacy Canvas context into provider turns", async () => {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-croki-context"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("user-message-croki-context"),
          role: "user",
          text: userText,
          attachments: [],
        },
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-croki-attachment-only"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("user-message-croki-attachment-only"),
          role: "user",
          text: "",
  ...
  it("stops the projected session and records the failure when provider interrupt fails", async () => {
    const harness = await createHarness({
      interruptTurnEffect: () =>
        Effect.fail(
          new ProviderAdapterRequestError({
  ...
  it("keeps ordinary sends FIFO and drains one after the active turn settles", async () => {
    const harness = await createHarness();
    const firstAt = "2026-01-01T00:00:00.000Z";
    const secondAt = "2026-01-01T00:00:01.000Z";

    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-lane-first"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("lane-first-message"),
          role: "user",
          text: "first",
          attachments: [],
        },
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-lane-running"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "running",
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-lane-second"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("lane-second-message"),
          role: "user",
          text: "second",
          attachments: [],
        },
    ...
    await waitFor(async () => {
      ...
      return (
        thread?.queuedTurnStarts?.map((entry) => entry.messageId).join() === "lane-second-message"
      );
    });
    expect(harness.sendTurn.mock.calls.length).toBe(1);
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-lane-interrupted"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "interrupted",
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-lane-next-running"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "running",
    ...
    await waitFor(async () => {
      ...
      return (thread?.queuedTurnStarts?.length ?? 0) === 0;
    });
    const finalThread = (await harness.readModel()).threads.find(
      (entry) => entry.id === ThreadId.make("thread-1"),
    );
    expect(finalThread?.queuedTurnStarts).toEqual([]);
  });
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* harness.engine.dispatch({
          ...
          threadId: ThreadId.make("thread-1"),
          activity: {
            id: EventId.make("approval-requested-for-response"),
            tone: "approval",
  ...
  effectIt.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* harness.engine.dispatch({
          type: "thread.activity.append",
          commandId: CommandId.make("cmd-user-input-requested-for-response"),
          threadId: ThreadId.make("thread-1"),
          activity: {
            id: EventId.make("user-input-requested-for-response"),
            tone: "info",
