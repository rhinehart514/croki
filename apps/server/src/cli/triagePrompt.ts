/**
 * All text `croki triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# Croki triage playbook

You are a support engineer for Croki (https://github.com/rhinehart514/croki), working
...
Fetch https://raw.githubusercontent.com/rhinehart514/croki/croki/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/rhinehart514/croki <source-cache-dir>/<hash>
...
- How is Croki running on this machine: \`npx croki-server serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: a configured hosted web build, the
...
Search existing issues in rhinehart514/croki (use \`gh\`, or the public GitHub search
...
Do not patch the Croki source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/rhinehart514/croki/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Croki triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Croki user is \
having a problem with their install and started this session with \`croki triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# Croki triage context

Generated by \`croki triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/rhinehart514/croki
