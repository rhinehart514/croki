...
import { HostProcessArguments } from "@croki/shared/hostProcess";
 ...
 * Detection is best-effort; callers must fail closed to a plain `croki` command.
 */
...
/**
 * The Croki package spec to suggest. The literal spec the user typed (e.g.
 * `croki-server@nightly`) is resolved away before our process starts, so re-derive it
...
export function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? "croki-server@nightly" : "croki-server";
}

/**
 * Render a `croki <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx croki-server connect` suggests
 * `npx croki-server serve`, a global install suggests `croki serve`, and a nightly build
