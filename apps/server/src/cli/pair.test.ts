...
describe("croki pair", () => {
  it.effect("mints a token and prints a QR pairing URL for a live server", () =>
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        ...
        assert.equal(credentials[0]?.label, "Croki pair");
      }),
  ...
  it.effect("directs to Croki serve or connect when no server is running", () =>
    Effect.gen(function* () {
      ...
      assert.include(rendered, "No running Croki server found.");
      assert.include(rendered, "npx croki-server serve");
      assert.include(rendered, "npx croki-server connect");
    }).pipe(Effect.provide(NodeServices.layer)),
  );
