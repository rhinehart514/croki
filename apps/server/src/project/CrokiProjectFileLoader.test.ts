...
const TestLayer = Layer.empty.pipe(
  Layer.provideMerge(CrokiProjectFileLoader.layer),
...
const writeProjectFile = Effect.fn("writeProjectFile")(function* (cwd: string, contents: string) {
  ...
  yield* fileSystem.writeFileString(path.join(cwd, "croki.json"), contents).pipe(Effect.orDie);
});

it.layer(TestLayer)("CrokiProjectFileLoader", (it) => {
  describe("load", () => {
    it.effect("loads and decodes a valid croki.json", () =>
      Effect.gen(function* () {
        const loader = yield* CrokiProjectFileLoader.CrokiProjectFileLoader;
    ...
    it.effect("returns none when croki.json is missing", () =>
      Effect.gen(function* () {
        const loader = yield* CrokiProjectFileLoader.CrokiProjectFileLoader;
        const cwd = yield* makeTempDir;
