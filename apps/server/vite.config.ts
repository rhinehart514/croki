...
export default mergeConfig(
  ...
  defineConfig({
    ...
    pack: {
      ...
      define: {
        __CROKI_BUILD_CHANNEL__: JSON.stringify(cliBuildChannel),
        __CROKI_BUILD_RELAY_URL__: JSON.stringify(repoEnv.CROKI_RELAY_URL?.trim() ?? ""),
        __CROKI_BUILD_CLERK_PUBLISHABLE_KEY__: JSON.stringify(
          repoEnv.CROKI_CLERK_PUBLISHABLE_KEY?.trim() ?? "",
        ),
        __CROKI_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: JSON.stringify(
          repoEnv.CROKI_CLERK_CLI_OAUTH_CLIENT_ID?.trim() ?? "",
        ),
        __CROKI_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: JSON.stringify(
          repoEnv.CROKI_RELAY_CLIENT_OTLP_TRACES_URL?.trim() ?? "",
        ),
        __CROKI_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: JSON.stringify(
          repoEnv.CROKI_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() ?? "",
        ),
        __CROKI_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: JSON.stringify(
          repoEnv.CROKI_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() ?? "",
        ),
      },
    },
    test: {
