...
function makeEnvironmentLayer(baseDir: string, env: Record<string, string | undefined> = {}) {
  return DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ CROKI_HOME: baseDir, ...env })),
    ),
  );
}
...
function makeLayer(input: {
  ...
  const env = { CROKI_HOME: input.baseDir, ...input.env };
...
describe("DesktopServerExposure", () => {
  ...
  it.effect("uses ConfigProvider desktop exposure overrides", () =>
    withHarness(
      ...
      {
        CROKI_DESKTOP_LAN_HOST: "10.0.0.7",
        CROKI_DESKTOP_HTTPS_ENDPOINTS: "https://public.example.test",
      },
    ),
  );
