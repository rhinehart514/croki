...
>()("@croki/desktop/backend/DesktopBackendConfiguration") {}
...
const DESKTOP_BACKEND_ENV_NAMES = [
  ...
  "CROKI_NO_BROWSER",
  "CROKI_HOST",
  "CROKI_DESKTOP_WS_URL",
  "CROKI_DESKTOP_LAN_ACCESS",
  "CROKI_DESKTOP_LAN_HOST",
  "CROKI_DESKTOP_HTTPS_ENDPOINTS",
  "CROKI_TAILSCALE_SERVE",
  "CROKI_TAILSCALE_SERVE_PORT",
] as const;
...
function resourceMonitorBinaryName(platform: NodeJS.Platform): string {
  return platform === "win32" ? "croki-resource-monitor.exe" : "croki-resource-monitor";
}
...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    return {
      ...
      // Primary wants process.env (PATH, dev-runner's CROKI_HOME, etc.).
      extendEnv: true,
      bootstrap,
...
> {
  ...
  // Build an explicit copy of process.env minus CROKI_HOME (dev-runner
  ...
  const baseConfig = {
    ...
    // env is already a complete process.env minus CROKI_HOME; pass it
