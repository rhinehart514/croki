...
const AppPackageMetadata = Schema.Struct({
  crokiCommitHash: Schema.optional(Schema.String),
});
...
>()("@croki/desktop/app/DesktopAppIdentity") {}
    ...
    return yield* Option.match(raw, {
      ...
      onSome: (value) =>
        decodeAppPackageMetadata(value).pipe(
          Effect.map((parsed) =>
            Option.fromNullishOr(parsed.crokiCommitHash).pipe(Option.flatMap(normalizeCommitHash)),
          ),
