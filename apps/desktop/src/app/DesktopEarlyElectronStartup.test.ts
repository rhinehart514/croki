...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("reads the persisted linux password-store preference before Electron is ready", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: { CROKI_HOME: "/home/user/.croki-test" },
      homeDirectory: "/home/user",
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.croki-test/userdata/desktop-settings.json");
  ...
  it("resolves the early linux Electron switches", () => {
    const options = resolveEarlyLinuxElectronOptions({
      env: {
        CROKI_HOME: "/home/user/.croki-test",
    ...
    assert.deepEqual(options, {
      linuxWmClass: "croki-dev",
      passwordStore: "gnome-libsecret",
    });
  });

  it("keeps implicit development state under ~/.croki/dev when CROKI_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.croki/dev/desktop-settings.json");
  ...
  it("treats whitespace-only CROKI_HOME as unconfigured in development", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        CROKI_HOME: "   ",
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
