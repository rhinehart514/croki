...
const defaultEnvironmentInput = {
  ...
  processArch: "arm64",
  appVersion: "1.2.3",
  appPath: "/Applications/Croki (Alpha).app/Contents/Resources/app.asar",
  isPackaged: true,
  resourcesPath: "/Applications/Croki (Alpha).app/Contents/Resources",
...
const makeElectronAppLayer = (calls: ElectronAppCalls) =>
  Layer.succeed(ElectronApp.ElectronApp, {
    ...
    name: Effect.succeed("Croki (Alpha)"),
...
describe("DesktopAppIdentity", () => {
  it.effect("always uses Croki-owned userData instead of sharing the T3 profile", () =>
    withIdentity(
      Effect.gen(function* () {
        const identity = yield* DesktopAppIdentity.DesktopAppIdentity;
        const userDataPath = yield* identity.resolveUserDataPath;

        assert.equal(userDataPath, "/Users/alice/Library/Application Support/Croki");
      }),
    ),
  );
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Croki"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Croki");
