...
const makeEnvironment = (overrides: Record<string, unknown> = {}) =>
  DesktopEnvironment.DesktopEnvironment.of({
    ...
    appImagePath: Option.some("/home/alice/Applications/Croki.AppImage"),
...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    const entry = DesktopLinuxUrlHandler.renderUrlHandlerDesktopEntry({
      ...
      execTarget: '/home/al ice/Apps/Croki "100%" $HOME\\x.AppImage',
      scheme: "croki",
    });

    assert.include(entry, "[Desktop Entry]");
    assert.include(entry, "Name=Croki (Nightly)");
    ...
    assert.include(
      entry,
      'Exec="/home/al ice/Apps/Croki \\\\"100%%\\\\" \\\\$HOME\\\\\\\\x.AppImage" %U',
    );
    assert.include(entry, "NoDisplay=true");
    assert.notInclude(entry, "StartupWMClass=");
    assert.include(entry, "MimeType=x-scheme-handler/croki;");
  });
  ...
  it("carries structured context on registration errors", () => {
    const writeError = new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
      ...
      scheme: "croki",
      desktopEntryPath: "/home/alice/.local/share/applications/croki-url-handler.desktop",
      cause: new Error("boom"),
    });
    assert.equal(
      writeError.message,
      "Failed to register the croki:// URL handler (step: write-desktop-entry).",
    );
    assert.equal(
      writeError.desktopEntryPath,
      "/home/alice/.local/share/applications/croki-url-handler.desktop",
    );
    ...
    assert.equal(
      exitError.message,
      "Failed to register the croki:// URL handler (step: set-default-handler, xdg-mime exit code 4).",
    );
  });
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(
        ...
        'Exec="/home/alice/Applications/Croki.AppImage" %U',
      );
      assert.include(recorded.files[0]?.content, "MimeType=x-scheme-handler/croki;");
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: ["default", "croki-url-handler.desktop", "x-scheme-handler/croki"],
        },
      ]);
    });
  });
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(writeFailed, {
        writeError: PlatformError.systemError({
          ...
          pathOrDescriptor: "/home/alice/.local/share/applications/croki-url-handler.desktop",
        }),
      });
