...
const shouldLaunchElectronAfterPack = process.env.CROKI_DESKTOP_DEV === "1";
  ...
  run: {
    tasks: {
      build: {
        ...
        dependsOn: ["croki-server#build"],
        cache: false,
      },
      dev: {
        command:
          "node scripts/build-preview-annotation-css.mjs && cross-env CROKI_DESKTOP_DEV=1 vp pack --watch",
        dependsOn: ["croki-server#build"],
        cache: false,
      },
      "dev:bundle": {
  ...
  pack: [
    {
      ...
      clean: true,
      deps: {
        alwaysBundle: (id) => id.startsWith("@croki/"),
      },
