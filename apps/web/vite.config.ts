...
const isSingleOriginDev = process.env.CROKI_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_CROKI_RELAY_URL?.trim() || "";
...
const configuredServerPackageUpdatesAvailable =
  process.env.VITE_CROKI_SERVER_PACKAGE_UPDATES_AVAILABLE?.trim().toLowerCase() ?? "";
...
const sourcemapEnv = process.env.CROKI_WEB_SOURCEMAP?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // CROKI_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const configuredAllowedHosts = (process.env.CROKI_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_CROKI_RELAY_URL": JSON.stringify(configuredRelayUrl),
      ...
      "import.meta.env.VITE_CROKI_SERVER_PACKAGE_UPDATES_AVAILABLE": JSON.stringify(
        configuredServerPackageUpdatesAvailable,
      ),
