...
/**
 * The user's Croki Connect environments from relay discovery, each with a
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through Croki Connect.`,
      });
      return;
    }
    ...
    const message =
      cause instanceof Error ? cause.message : "Could not connect the Croki Connect environment.";
    const traceId = findErrorTraceId(cause);
    console.error("[croki-connect] Could not connect environment", { message, traceId, cause });
    toastManager.add({
      type: "error",
