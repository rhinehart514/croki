import type { VcsStatusResult } from "@croki/contracts";
...
describe("resolveLiveThreadBranchUpdate", () => {
  ...
  it("does not regress a semantic thread ref back to a temporary worktree ref", () => {
    const update = resolveLiveThreadBranchUpdate({
      threadBranch: "croki/github-query-rate-limit",
      gitStatus: status({ refName: "croki/bda76797" }),
    });
