import { relayClerkTokenOptions } from "@croki/shared/relayAuth";
...
export class CloudPublicConfigMissingError extends Schema.TaggedErrorClass<CloudPublicConfigMissingError>()(
  ...
  {
    key: Schema.Literal("CROKI_CLERK_JWT_TEMPLATE"),
  },
) {
...
export function resolveCloudPublicConfig(): CloudPublicConfig {
  return {
    ...
    relayUrl: normalizeSecureRelayUrl(
      (import.meta.env.VITE_CROKI_RELAY_URL as string | undefined) ?? "",
    ),
    relayTracing: {
...
export function resolveRelayClerkTokenOptions() {
  ...
  if (!clerkJwtTemplate) {
    throw new CloudPublicConfigMissingError({ key: "CROKI_CLERK_JWT_TEMPLATE" });
  }
