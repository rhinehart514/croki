...
/**
 * Drives the primary environment's Croki Connect link. Croki Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message =
      cause instanceof Error ? cause.message : "Could not update Croki Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[croki-connect] Could not update Croki Connect", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update Croki Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Croki Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Croki Connect before enabling this."));
        return false;
      }
