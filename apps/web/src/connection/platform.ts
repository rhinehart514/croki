...
} from "@croki/client-runtime/platform";
import {
...
import { bootstrapRemoteBearerSession } from "@croki/client-runtime/authorization";
import { fetchRemoteEnvironmentDescriptor } from "@croki/client-runtime/environment";
import { managedRelayAccountChanges, managedRelaySessionAtom } from "@croki/client-runtime/relay";
import { EnvironmentRpcRequestObserver } from "@croki/client-runtime/rpc";
import {
...
function clientMetadata() {
  ...
  return {
    label: desktop ? "Croki Desktop" : "Croki Web",
    ...
    const cloudSession = CloudSession.of({
      clerkToken: Effect.gen(function* () {
        ...
        if (session === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "Sign in to Croki Connect to connect this environment.",
          });
        }
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The Croki Connect session is unavailable.",
          });
        }
        return token;
      }),
