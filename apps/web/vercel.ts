...
const HOSTED_WEB_CHANNEL_COOKIE = "croki_web_channel";
...
export const config: VercelConfig = {
  buildCommand:
    'vp run --filter @croki/web build && node ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "npm install -g vite-plus && vp install --ignore-scripts --filter '@croki/scripts...' --filter '@croki/web...'",
  routes: [
    {
      src: "/__croki/channel",
