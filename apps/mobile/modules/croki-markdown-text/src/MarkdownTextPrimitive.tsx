...
import CrokiMarkdownTextRunNativeComponent from "./CrokiMarkdownTextRunNativeComponent";
import CrokiMarkdownTextNativeComponent from "./CrokiMarkdownTextNativeComponent";
...
function MarkdownTextPrimitiveChild({ style, children, ...rest }: MarkdownTextPrimitiveProps) {
  ...
  const nativeChildren = React.Children.toArray(children).map((child) => {
    ...
    return (
      ...
      <CrokiMarkdownTextRunNativeComponent
  ...
  if (!isAncestor) {
    return (
      <TextAncestorContext.Provider value={contextValue}>
        <CrokiMarkdownTextNativeComponent
          {...textDefaults}
          {...rest}
