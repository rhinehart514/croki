...
#import "CrokiMarkdownTextShadowNode.h"
#import "CrokiMarkdownTextComponentDescriptor.h"
...
static void CrokiMarkdownTextApplyParagraphStyles(
...
static void CrokiMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
...
@interface CrokiMarkdownText () <RCTCrokiMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface CrokiMarkdownText () <T3MarkdownOutsideTapTarget>
@end

@implementation CrokiMarkdownText {
  UIView * _view;
  UITextView * _textView;
  CrokiMarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<CrokiMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const CrokiMarkdownTextProps>();
    _props = defaultProps;
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<CrokiMarkdownTextProps const>(_props);
  ...
  CrokiMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  CrokiMarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::CrokiMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::CrokiMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<CrokiMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
{
  const auto &oldViewProps = *std::static_pointer_cast<CrokiMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<CrokiMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == CrokiMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == CrokiMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == CrokiMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == CrokiMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const CrokiMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (CrokiMarkdownTextRun*)getTouchChild:(CGPoint)location
{
  ...
  for (UIView* child in self.subviews) {
    if (![child isKindOfClass:[CrokiMarkdownTextRun class]]) {
      continue;
    }

    CrokiMarkdownTextRun* textChild = (CrokiMarkdownTextRun*)child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::CrokiMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::CrokiMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> CrokiMarkdownTextCls(void)
{
  return CrokiMarkdownText.class;
}
