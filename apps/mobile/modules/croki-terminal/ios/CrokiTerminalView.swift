...
public final class CrokiTerminalView: ExpoView, UITextFieldDelegate {
  ...
  var terminalKey: String = "" {
    didSet {
      accessibilityIdentifier = "croki-terminal-\(terminalKey)"
  ...
  private func setupWriteCallback() {
    ...
    ghostty_surface_set_write_callback(surface, { userdata, data, len in
      ...
      let view = Unmanaged<CrokiTerminalView>.fromOpaque(userdata).takeUnretainedValue()
  ...
  private func writeThemeConfigFile() -> String? {
    ...
    let url = URL(fileURLWithPath: NSTemporaryDirectory())
      .appendingPathComponent("croki-terminal-theme-\(appearance.rawValue).ghostty")
