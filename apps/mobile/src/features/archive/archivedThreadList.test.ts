import type { ArchivedSnapshotEntry } from "@croki/client-runtime/state/threads";
import type { OrchestrationProjectShell, OrchestrationThreadShell } from "@croki/contracts";
...
describe("buildArchivedThreadGroups", () => {
  it("groups archived threads by project and sorts newest first", () => {
    const project = makeProject({ id: ProjectId.make("project-1"), title: "Croki" });
  ...
  it("filters by environment and matches project, thread, and branch text", () => {
    ...
    const firstProject = makeProject({ id: ProjectId.make("project-1"), title: "Croki" });
