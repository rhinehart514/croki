...
enum PlatformRoute: Codable, Hashable, Identifiable, Sendable {
    static let nativeScheme = "croki-native"
...
enum PlatformDeepLinkError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        switch self {
        case .unsupportedURL:
            "That Croki link is not supported."
        case .missingIdentifier:
            "That Croki link is missing its destination."
        case .invalidIdentifier:
            "That Croki link contains an invalid destination."
        }
    }
}
