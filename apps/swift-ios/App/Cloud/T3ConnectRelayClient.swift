...
public enum T3ConnectRelayError: LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        ...
        case let .invalidConfiguration(message):
            message
        case .invalidResponse:
            "Croki Connect returned an invalid response."
        ...
        case let .unexpectedScope(requested, granted):
            "Croki Connect granted \(granted) instead of \(requested.joined(separator: " "))."
        case .environmentMismatch:
            "Croki Connect returned credentials for a different environment."
        }
    }
}
    ...
    public func registerDevice(
        ...
        guard registration.iosMajorVersion >= 18 else {
            throw T3ConnectRelayError.invalidConfiguration(
                "Croki Connect device registration requires iOS 18 or newer."
            )
        }
    ...
    private func send<Response: Decodable & Sendable>(
        ...
        guard (200..<300).contains(response.statusCode) else {
            ...
            throw T3ConnectRelayError.response(
                ...
                message: body?.message ?? body?.reason ?? body?.code ?? "Croki Connect request failed.",
