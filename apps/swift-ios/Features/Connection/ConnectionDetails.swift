...
enum ConnectionDetailsError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        switch self {
        case .empty:
            "Paste a Croki pairing link or enter your server address."
        case .invalidAddress:
            "That connection link does not include a valid server address."
        case .unsupportedScheme:
            "Use an HTTP or HTTPS Croki server address."
        }
    }
}

/// Parses the pairing links emitted by local, shared, and hosted Croki environments.
enum ConnectionDetailsParser {
    private static let applicationSchemes = Set([
        "croki",
        "croki-dev",
        "croki-preview",
        "croki-native",
    ...
    static func parse(_ input: String) throws -> ConnectionDetails {
        ...
        let extracted = applicationSchemes.contains(where: {
        ...
        if candidate.contains("://")
            || applicationSchemes.contains(where: { loweredCandidate.hasPrefix("\($0):") }) {
    ...
    private static func parseURL(_ input: String) throws -> ConnectionDetails {
        ...
        if applicationSchemes.contains(scheme) {
    ...
    private static func firstURL(in input: String) -> String? {
        guard let expression = try? NSRegularExpression(
            pattern: #"(?i)(?:(?:https?|wss?)://|(?:croki(?:-(?:dev|preview|native))?|t3|t3code):(?://)?)[^\s<>"']+"#
        ) else {
            return nil
        }
...
enum ConnectionErrorCopy {
    static func message(for rawMessage: String?) -> String {
        ...
        if message.isEmpty || message == "cancelled" || message == "canceled" {
            return "The connection stopped before it finished. Make sure Croki is running, then try again."
        }
        ...
        return "Croki could not complete pairing. Check the server address and use a fresh pairing code."
    }
}
