...
struct PlatformDeepLinkTests {
    ...
    func parsesWidgetThreadRoute() throws {
        let route = try PlatformDeepLinkParser.parse(
            "croki-native://threads/environment-1/thread-7"
        )
    ...
    func parsesProjectAndEnvironmentQueryRoutes() throws {
        #expect(
            try PlatformDeepLinkParser.parse("croki-native://projects/project-7?environment=environment-1")
                == .project(environmentID: "environment-1", projectID: "project-7")
        )
        #expect(
            try PlatformDeepLinkParser.parse("croki-native://environments/environment-2")
                == .environment(id: "environment-2")
        )
        #expect(
            try PlatformDeepLinkParser.parse("croki-native://new-task?environment=environment-2&project=project-8")
    ...
    func unwrapsPairingURL() throws {
        let route = try PlatformDeepLinkParser.parse(
            "croki-native://pair?pairingUrl=https%3A%2F%2Fremote.example.com%2Fpair%23token%3DPAIR"
        )
    ...
    func acceptsLegacyRoutesWithoutClaimingTheirSharedSchemes() throws {
        #expect(
            try PlatformDeepLinkParser.parse("croki://threads/environment/thread")
