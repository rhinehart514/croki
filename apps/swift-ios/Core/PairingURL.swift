...
public enum PairingURLError: LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        ...
        case .invalidQRCode: "Scanned QR code is not a Croki pairing link."
        }
    }
}
...
public enum PairingURL {
    ...
    private static let wrapperSchemes = Set([
        "croki",
        "croki-dev",
        "croki-preview",
        "croki-native",
    ...
    /// QR scanner. Croki wrappers and legacy T3 wrappers are unwrapped.
    ...
    private static func extractPairingURL(
        ...
        guard let components = URLComponents(string: trimmed),
              wrapperSchemes.contains(components.scheme?.lowercased() ?? "")
        else {
            return trimmed
        }
