...
export type ObservationContract = { id: string; type: "observation-contract"; releaseRef: string; source: "gmail"; purpose: string; startsAt: string; endsAt: string; returnConditions: string[]; messageIds: string[]; decisionRefs: string[]; grantedAt: string; grantedBy: string; revokedAt: string | null; lastCheckedAt: string | null; lastResult: { polled?: number; ingested?: Array<Record<string, unknown>>; reason?: string; needsReconnect?: boolean } | null };
export type ReleaseSummary = { id: string; releaseRef: string; name: string; statement: string; lifecycle: ReleaseLifecycle; attention: ReleaseAttention[]; updatedAt: string | null; createdAt: string | null; endedAt: string | null; endedBy: string | null; relatedObjectRefs: string[]; threadRefs: string[]; runRefs: string[]; threads?: Array<Record<string, unknown>>; runs?: Array<Record<string, unknown>>; receipts?: Array<Record<string, unknown>>; decisions: Array<Record<string, unknown>>; outcomes: Array<Record<string, unknown>>; observations: ObservationContract[]; relationships: SystemIndexRelationship[]; externalRefs: Record<string, string[]> };
...
export const grantReleaseObservation = (ventureId: string, releaseId: string, body: { source: "gmail"; purpose: string; startsAt: string; endsAt: string; returnConditions: string[] }) => guardedPost<{ contract: ObservationContract }>(`/api/ventures/${encodeURIComponent(ventureId)}/releases/${encodeURIComponent(releaseId)}/observation`, body);
export const checkReleaseObservation = (ventureId: string, releaseId: string, contractId: string) => guardedPost<{ contract: ObservationContract; evidence: { polled: number; ingested: Array<Record<string, unknown>>; reason?: string; needsReconnect?: boolean } }>(`/api/ventures/${encodeURIComponent(ventureId)}/releases/${encodeURIComponent(releaseId)}/observation/${encodeURIComponent(contractId)}/check`, {});
export const revokeReleaseObservation = (ventureId: string, releaseId: string, contractId: string) => guardedPost<{ contract: ObservationContract }>(`/api/ventures/${encodeURIComponent(ventureId)}/releases/${encodeURIComponent(releaseId)}/observation/${encodeURIComponent(contractId)}/revoke`, {});
