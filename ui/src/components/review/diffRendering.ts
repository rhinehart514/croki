...
/** Stable content-addressed cache key so Pierre's AST LRU cache survives re-renders. */
...
/**
 * Parse a unified/git patch into Pierre file metadata. Empty input yields null; text that is not a
 * recognizable diff falls back to the exact raw text with an honest reason, never a crash.
 */
export function getRenderablePatch(
  ...
  try {
    ...
    return {
      ...
      reason: "This isn’t a readable diff, so the exact text is shown.",
    };
  } catch {
    return {
      kind: "raw",
      text: normalizedPatch,
      reason: "This diff couldn’t be parsed, so the exact text is shown.",
    };
  }
}
...
/** Added/removed line totals for one file, derived from the parsed hunks. */
export function fileDiffStats(fileDiff: FileDiffMetadata): { additions: number; deletions: number } {
  let additions = 0;
  let deletions = 0;
