...
export function AgentPurposeDialog({ member, agent, workflowNames, onClose, onWork }: {
  ...
  return <Dialog.Root open onOpenChange={(open) => { if (!open) onClose(); }}>
    <Dialog.Portal><Dialog.Backdrop className="agent-purpose-backdrop" /><Dialog.Viewport className="agent-purpose-viewport">
      <Dialog.Popup render={<motion.section className="agent-purpose-dialog" initial={reducedMotion ? false : { opacity: 0, scale: 0.97, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: reducedMotion ? 0 : 0.2, ease: [0.22, 1, 0.36, 1] }} />}>
