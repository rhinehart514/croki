...
import { canvasReservedWidth, foldPlacement, seedCanvasPositions } from "./canvasSeedLayout";
...
describe("seedCanvasPositions — collision-free by construction at the arrival fit", () => {
  ...
  // Heights are the archetype artifact heights; the seed reserves those too. Mirrored minimally here just
  // to build the AABB — width (the breach dimension) comes from the shared truth above.
  const heightFor = (kind: string) => {
    if (kind === "intent" || kind === "theory") return 280;
    if (kind === "bet" || kind === "work" || kind === "outcome") return 280;
    if (kind === "capability") return 200;
    if (kind === "teammate") return 220;
    return 270; // architecture family (product-loop/system/campaign/motion)
  };
