// Engine + model catalog for the AgentPicker. Kept in a non-component module so AgentPicker.tsx
// only exports components (fast-refresh requires that), and so ComposerDock can read the model
// helpers without pulling in the component.
//
...
// Both marks are inlined as single SVG paths. The Claude mark is the canonical simple-icons brand
// path (claude.svg, hex D97757); OpenAI is trademark-excluded from simple-icons so its mark is
// inlined too. Inlining these two avoids importing the whole ~5.5MB simple-icons barrel into this
// eager module — the full dataset stays in its own lazy-loaded chunk (see lib/brandGlyph.ts).
const CLAUDE_PATH =
...
export type AgentId = "auto" | "claude" | "codex";

export type Engine = { id: AgentId; name: string; meta: string; path: string; brand: string };

export const ENGINES: Record<AgentId, Engine> = {
  auto: { id: "auto", name: "Auto", meta: "Best available", path: "M12 2 14.4 9.6 22 12l-7.6 2.4L12 22l-2.4-7.6L2 12l7.6-2.4L12 2Z", brand: "#52525B" },
  claude: { id: "claude", name: "Claude", meta: "Your subscription", path: CLAUDE_PATH, brand: `#${CLAUDE_HEX}` },
...
// Current models (June 2026). Claude: Opus 4.8 / Sonnet 4.6, with explicitly chosen Fable 5 under More.
...
export const MODELS: Model[] = [
  { id: "auto", agent: "auto", label: "Auto", note: "Best available runtime", tier: "primary" },
