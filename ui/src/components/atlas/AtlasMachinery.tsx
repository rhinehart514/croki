  ...
  return (
    <aside data-atlas-machinery className="atlas-machinery" aria-label="Execution details">
      ...
      <dl>
        ...
        <div className="atlas-machinery-teammates"><dt>Teammates</dt><dd>{teammates.length ? <span>{teammates.slice(0, 3).map((teammate) => <CrewFace key={teammate.ref} agentRef={teammate.ref} size={26} />)}<b>{teammates.map((teammate) => teammate.name).join(" + ")}</b></span> : "None configured"}</dd></div>
