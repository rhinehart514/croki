...
export function ThreadComposer({ ventureId, ventureName, repository, surface = "context", contextKind = null, item, lens, draft, isHome, readOnly, readOnlyReason, subjectRefs = [], scopeLabel, targetAgentRef = null, workRef = null, workflowSketch = false, artifactFocus = null, artifactFocusRequest = 0, onClearArtifactFocus, workChatMode = "code", onWorkChatModeChange = () => undefined, onSubmitStart, onSubmitFailed, onDriven, onWorkRouted }: {
  ...
  const placeholder = targetAgent
    ...
    : agentChat ? "Ideate a Product / GTM workflow…"
    : artifactFocus ? `Change “${artifactFocus.sectionTitle}”…`
    : surface === "work" ? draft || isHome ? "Describe what you want to build…" : workflowSketch ? "Correct this workflow…" : "Ask for a change…" : contextKind === "product-gtm" ? "Describe the Product or go-to-market mechanism…" : "Ask about this…";
  const submissionMode = agentChat ? "product-gtm" : surface === "work" ? "work" : targetAgent ? "auto" : "conversation";
  return <div className="thread-composer" data-chat-mode={surface === "work" ? workChatMode : undefined}><NowComposer ventureId={ventureId} ventureName={ventureName} selection={selection} subjectRefs={subjectRefs} scopeLabel={targetAgent?.name ?? scopeLabel ?? (draft || isHome ? null : item?.founderIntent ?? null)} hasWork={Boolean(lens?.bets.length)} variant="dock" submissionMode={submissionMode} readOnly={readOnly} readOnlyReason={readOnlyReason} autoFocus={draft} focusRequest={artifactFocusRequest} placeholder={placeholder} runtimeOverride={agentChat ? null : targetAgent?.runtime.provider ?? modelChoice.runtime} modelOverride={agentChat ? null : targetAgent?.runtime.model ?? modelChoice.model} composerControls={controls} workflowSketch={agentChat || workflowSketch || contextKind === "product-gtm"} artifactSection={artifactFocus} onClearArtifactSection={onClearArtifactFocus} onSubmitStart={onSubmitStart} onSubmitFailed={onSubmitFailed} onDriven={onDriven} onWorkRouted={onWorkRouted} /></div>;
}
