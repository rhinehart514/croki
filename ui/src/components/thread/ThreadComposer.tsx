...
export function ThreadComposer({ ventureId, ventureName, repository, surface = "context", contextKind = null, item, lens, draft, isHome, readOnly, readOnlyReason, subjectRefs = [], scopeLabel, targetAgentRef = null, workflowStep = null, workRef = null, productGtmView = false, workflowSketch = false, modelBranchRef = null, artifactFocus = null, artifactFocusRequest = 0, onClearArtifactFocus, workChatMode = "code", onWorkChatModeChange = () => undefined, modelChoice, onModelChoice, activeDrives = [], onSubmitStart, onSubmitAccepted, onSubmitFailed, onDriven, onWorkRouted }: {
  ...
  const controls = surface === "work" && repository ? <WorkComposerBar key={threadKey} ventureId={ventureId} threadKey={threadKey} repository={repository} disabled={readOnly} mode={workChatMode} onModeChange={onWorkChatModeChange} onChange={onModelChoice} /> : null;
