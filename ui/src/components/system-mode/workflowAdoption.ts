...
export async function adoptWorkflowSketch(ventureId: string, index: SystemIndex, sketch: WorkflowSketch) {
  ...
  const id = existing?.id ?? `workflow-${crypto.randomUUID()}`;
  const statement = first?.detail ?? "A founder-adopted Product / GTM workflow.";
  const properties = {
  ...
  const result = await mutateSystem(ventureId, index.revision, [existing ? {
    ...
    op: "create-object", id, type: "pipeline", territory: "gtm", name: sketch.title, statement, properties,
  }]);
