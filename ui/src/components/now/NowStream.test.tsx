...
const directions: Direction[] = [
  direction({ id: "d1", sentence: "Contact these roofing operators", state: "needs-you", understanding: "An outward act to ops@acme.com is waiting for your release." }),
  direction({ id: "d2", sentence: "Find out why signups stalled and fix what you can", state: "working", understanding: "2 drafts are ready to inspect.", approaches: 3, proofCount: 2 }),
];

describe("NowStream", () => {
  it("renders one compact row per founder direction, titled by the sentence", () => {
    render(<NowStream directions={directions} now={Date.parse("2026-01-01T00:10:00Z")} onSelect={() => {}} />);
    const row = screen.getByRole("button", { name: /Find out why signups stalled/ });
    expect(row).toHaveAttribute("data-state", "working");
    // Machinery summaries (N approaches) are not shown to the founder.
  ...
  it("calls back with the direction when selected", () => {
    ...
    render(<NowStream directions={directions} now={Date.now()} onSelect={onSelect} />);
  ...
  it("shows the quiet first-run state when nothing is living", () => {
    render(<NowStream directions={[]} now={Date.now()} onSelect={() => {}} />);
