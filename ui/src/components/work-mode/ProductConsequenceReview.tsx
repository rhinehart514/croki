...
export function ProductConsequenceReview({ ventureId, workspace, readOnlyReason, busy, onBusy, onError, onChanged }: {
  ...
  return <section className="code-workspace-section code-workspace-product" data-review={review}>
    <header><span>Optional Canvas consequence</span><strong>{REVIEW_LABEL[review] ?? humanize(review)}</strong></header>
    <p className="code-workspace-product-intro">This source-bearing interpretation is separate from code verification. It changes Canvas truth only if you adopt it.</p>
    <label>
    ...
    <label className="code-workspace-product-question">
      ...
      <textarea aria-label="Evidence question" value={releaseQuestion} onChange={(event) => setReleaseQuestion(event.target.value)} disabled={Boolean(readOnlyReason || busy)} />
    </label>
    {readOnlyReason ? <p>{readOnlyReason}</p> : null}
    <div className="code-workspace-action-row">
      <button type="button" disabled={disabled} onClick={() => void decide("adopt")}>{review === "adopted" ? "Update adopted consequence" : "Adopt Canvas consequence"}</button>
      {review !== "adopted" ? <>
