...
}).annotate({ description: "Links an authenticated cloud user to a Croki environment." });
...
>()("@croki/contracts/relay/RelayClientPrincipal") {}
...
>()("@croki/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in Croki Connect user.",
  ),
);
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "Croki Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking Croki environments, connecting authorized clients, and publishing agent activity.",
  );
