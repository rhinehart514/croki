...
describe("ssh auth", () => {
  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      assert.equal(env.CROKI_SSH_AUTH_SECRET, "super-secret");
      assert.equal(env.DISPLAY, "croki");
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      const descriptor = yield* buildSshAskpassHelperDescriptor({
        directory: "C:\\temp\\croki-ssh-askpass",
      }).pipe(
        Effect.provide(
      ...
      assert.equal(descriptor.launcherPath, "C:\\temp\\croki-ssh-askpass\\ssh-askpass.cmd");
      assert.deepEqual(
