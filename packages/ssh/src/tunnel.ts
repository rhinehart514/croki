import type { DesktopSshEnvironmentBootstrap, DesktopSshEnvironmentTarget } from "@croki/contracts";
import {
  describeReadinessCause,
  waitForHttpReady as waitForHttpReadyShared,
} from "@croki/shared/httpReadiness";
import * as NetService from "@croki/shared/Net";
import { extractJsonObject, fromLenientJson } from "@croki/shared/schemaJson";
import { satisfiesSemverRange } from "@croki/shared/semver";
...
remote_node_satisfies_engine() {
  CROKI_NODE_ENGINE_RANGE=@@CROKI_NODE_ENGINE_RANGE@@
  if [ -z "$CROKI_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$CROKI_NODE_ENGINE_RANGE" <<'NODE'
@@CROKI_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for CROKI_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$CROKI_NODE_BIN/node" ]; then
        PATH="$CROKI_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@CROKI_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
CROKI_NODE_SCRIPT_PATH=@@CROKI_NODE_SCRIPT_PATH@@
if [ -n "$CROKI_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$CROKI_NODE_SCRIPT_PATH" "$@"
fi
...
require_installed_t3_cli() {
  ...
  printf 'Remote host installed %s but npm produced no t3 executable, which usually means a native dependency (node-pty) failed to build. Install a C toolchain on the remote host (Debian/Ubuntu: build-essential, Fedora/RHEL: gcc-c++ make, macOS: xcode-select --install) and try again.\\n' @@CROKI_PACKAGE_SPEC@@ >&2
  return 1
}
if command -v npx >/dev/null 2>&1; then
  require_installed_t3_cli npx --yes --package @@CROKI_PACKAGE_SPEC@@ || exit 1
  exec npx --yes @@CROKI_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  require_installed_t3_cli npm exec --yes --package @@CROKI_PACKAGE_SPEC@@ || exit 1
  exec npm exec --yes @@CROKI_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the t3 CLI and could not install @@CROKI_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
pick_port() {
  node - "$PORT_FILE" "@@CROKI_DEFAULT_REMOTE_PORT@@" "@@CROKI_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@CROKI_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@CROKI_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@CROKI_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@CROKI_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@CROKI_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@CROKI_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env CROKI_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@CROKI_READY_TIMEOUT_MS@@"; then
    printf 'Remote Croki server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
    if [ -s "$LOG_FILE" ]; then
...
STATE_DIR="$HOME/.t3/ssh-launch/@@CROKI_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.t3"
RUNNER_FILE="$STATE_DIR/run-t3.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteT3RunnerScript(input?: RemoteT3RunnerOptions): string {
  ...
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      CROKI_PACKAGE_SPEC: packageSpec,
      CROKI_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      CROKI_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}
...
export function buildRemoteNodeEnvScript(input?: RemoteT3RunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      CROKI_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      CROKI_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}
...
export function buildRemoteLaunchScript(input?: RemoteT3RunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    CROKI_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    CROKI_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteT3RunnerScript(input)),
    CROKI_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    CROKI_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    CROKI_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    CROKI_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    CROKI_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    CROKI_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    CROKI_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}
...
export function buildRemotePairingScript(
  ...
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    CROKI_STATE_KEY: remoteStateKey(target),
    CROKI_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteT3RunnerScript(input)),
  });
}
...
>()("@croki/ssh/tunnel/SshEnvironmentManager") {
